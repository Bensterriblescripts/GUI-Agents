...
mod tests {
    ...
    fn build_resume_context_preserves_turns_and_reasoning() {
        let output =
            "first prompt\n\n\x1Ethinking\nanswer\n\nsecond prompt\n\n\x1FPlanning...\nfinal";
        let first = output.find("first prompt").unwrap();
        let second = output.find("second prompt").unwrap();
        let prompt_ranges = vec![
            (first, first + "first prompt".len()),
            (second, second + "second prompt".len()),
        ];

        let transcript = build_resume_context(output, &prompt_ranges);
